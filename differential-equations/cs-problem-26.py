...
#
# A screensaver drawing trajectories following the Lorenz equation.
...
# The camera shooting the scene. Inherits
...
class Camera (vtk.vtkCamera) :
    ...
    # to the x,y,z values of the focusPointDet object.
    # 
...
#
# The class is responsible for drawing a trajectory following the Lorenz
# equation and thus producing a visual representation of the Lorenz
# attractor in the 3D space. Extends vtkAssembly  which acts as a container
# for  the computed line segments.
#
class LorenzTrajectory (vtk.vtkAssembly) :

    STEP_SIZE = 0.015       # Step size used to compute next position
    ...
    #
    # For the current x,y,z position the method computes
...
#
# Represents the Lorenz attractor screensaver.
#
class ScreenSaver ():
    ...
    def __init__(self) :
        ...
        # Cerate the camera observing the scene
        self.camera = Camera()
      

    #
    # Puts trajectories and the camera into the scene
    #
    def setUpScene (self) :
        # Init the trajectory
    ...
    def update (self, obj,event) :
        ...
        self.iteration += 1
        # Reset the scene after some iterations (rendering becomes too slow otherwise) 
        if self.iteration % self.MAX_ITERATIONS == 0 : 
            self.resetScene()
    
# Setup and stat the screensaver.
